...
@Component({
  selector: 'app-wysig',
  template: '<textarea [ngModel]="prefill" id="{{elementId}}"></textarea>'
})
export class WysigComponent implements  AfterViewInit, OnDestroy {
  ...
  /**
   * Initialisation of the TINYMCE wysig Component - The whole Config is custom made
   */
  ngAfterViewInit() {

    // Some Timehacking for setting up the TinyMCE
    setTimeout(() => {
      tinymce.init({
        selector: '#' + this.elementId,
        plugins: ['link', 'paste', 'table', 'lists', 'advlist', 'code'],
        ...
        elementpath: false,
        content_css: '../assets/layout.css',
        language_url: '../assets/nl.js',
        menubar: false,
        setup: editor => {
          this.editor = editor;
          editor.on('keyup', () => {
            // this.serser.debugLog('FromTINY: ' + this.prefill);
            this.content = editor.getContent();
            this.onEditorKeyup.emit(this.content);
            const ed = tinymce.get(this.elementId); // get editor instance
            const range = ed.selection.getRng(); // get range
            this.carretPosition.emit(range);
           // console.log(range);
          });
          editor.on('change', () => {
            // check if not null
   ...
   /**
   *  Function Called from parent Component
   * Emit the value to this component and update the value of the content
   */
  callFromParent(_updatecontent) {
    // console.log('tinyfromparent' + '_updatecontent');
    this.content = this.editor.setContent(_updatecontent);
    this.content = this.editor.getContent();
    this.onEditorKeyup.emit(this.content);
  }

  /**
   *  Destroy this instance when OnDestroy is called
   */
  ngOnDestroy() {
    // dirty boi
